...
async def main():
   async with app: 
     ...
     while upl:
            ...
            await app.edit_message_text(-1002097863599,5,text=stats("Active",link,))
                        ...
                        for  filename in files[0]:
                            if filename not in filenames:
                                try:
                                        ...
                                        if len(imgs) >0:
                                          pic = await app.send_media_group(-1001585702100,imgs)
                                          time.sleep(2)
                                          pic = pic[0]
                                        else:
                                           os.system(f'''vcsi """{filename}""" -g 3x3--metadata-position hidden -o """{filename.replace('.mp4','.jpeg')}""" ''')
                                           pic = await app.send_photo(-1001585702100,photo=filename.replace('.mp4','.jpeg'))
                                           gof.upload(filename.replace('.mp4','.jpeg'),clip_folder["id"])
                                           
                                        sample = await app.send_video(-1001585702100,video=sample_filename,caption=sample_filename.replace(".mp4",""),thumb=sample_filename.replace("mp4","png"),supports_streaming=True,duration=int(duration(sample_filename)[0]),progress=progress)
                                        time.sleep(2)
                                        video = await app.send_video(-1001585702100,video=filename,caption=filename.replace(".mp4",""),thumb=filename.replace(".mp4",".jpg"),supports_streaming=True,duration=int(duration(filename)[0]),progress=progress)
                                        gf = gof.upload(filename,folder["id"])
                                        


                                        await app.send_photo(-1001848025191,photo=filename.replace('.mp4','.png'),caption=cap(filename,pic.id,video.id,sample.id,gf['downloadPage']),parse_mode=enums.ParseMode.MARKDOWN,progress=progress)
                                        ...
                                        print(e)
                                        await app.edit_message_text(-1002097863599,5,text=stats("Completed",link,e))
                    if up["Total"] > 15:
                            upl = False
            ...
            await app.edit_message_text(-1002097863599,5,text=stats("Offline",link,up["Total"]))
